...
### 目检事件 3：文档页"点击无反应"（2026-09-23）

- 现象：文档页标题/内容为空时点"创建文档"无反应（截图输入框为空）。
- 根因：VM `create()` 对空标题**静默 return**；`.error` 样式类在 globals.css 中不存在；mutation 错误没有展示位。
- 修复（frontend）：标题/内容分别给"请先填写标题/内容"提示（输入即清除）；globals.css 新增 `.error` 样式；批次页错误改用同一类。
- 验证（真实浏览器）：空标题→提示 ✓；缺内容→提示 ✓；填齐→`POST /docs 201` ✓；`tsc --noEmit` 通过。
- 数据清理：软删测试/重复文档 4 篇（目检探针 A/B、重复的"入驻审核规则" v1/v2），保留干净演示集 3 篇（入驻审核规则 v3 / 批处理失败处理 / 房源资料口径）。
- 备注：用户控制台 VM306 `reportAllChanges` 报错来自浏览器扩展注入脚本，与本项目无关。

### 交付前调整：09/quiz 移出仓库（2026-09-23）
