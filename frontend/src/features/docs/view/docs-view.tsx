  ...
  return (
    <section className="panel" style={{padding: 16}}>
      <div className="toolbar">
        <input className="field" placeholder="标题" value={vm.title} onChange={(event) => vm.setTitle(event.target.value)} />
        <input className="field" placeholder="内容" value={vm.content} onChange={(event) => vm.setContent(event.target.value)} />
        ...
        <tbody>
          {vm.docs.map((item) => (
            <tr key={item.id}>
              ...
              <td>
                <button className="icon-button" title="生成新版本" onClick={() => vm.update(item.id)}>
                  <Undo2 size={16} />
                </button>
